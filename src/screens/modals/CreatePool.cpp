...
CreatePool::CreatePool(const payload::Coin &coin, AccountController *ctrl) {
    ...
    auto *coinRow =
        (new qontrol::Row)->push(outpoint)->pushSpacer(10)->push(amount)->pushSpacer(10)->push(btcLabel)->pushSpacer();
    ...
    connect(m_w_denomination, &QLineEdit::editingFinished, this, [this]() { this->process(true); });
    ...
    connect(m_w_fee, &QLineEdit::editingFinished, this, [this]() { this->process(); });
    ...
    m_w_peers->setText("5");
    connect(m_w_peers, &QLineEdit::textEdited, this, [this]() { this->process(); });

    auto *peersRow = (new qontrol::Row)->pushSpacer(CP_L_SPACER)->push(peersLabel)->push(m_w_peers)->pushSpacer();
    ...
    connect(m_w_timeout, &QLineEdit::textEdited, this, [this]() { this->process(); });
    ...
    auto *btnRow = (new qontrol::Row)->pushSpacer()->push(createBtn)->pushSpacer(100)->push(cancelBtn)->pushSpacer();
...
void CreatePool::onCreatePool() {
    ...
    emit this->createPool(m_coin.outpoint, m_denomination, m_fees, m_max_duration, m_peers);
}
